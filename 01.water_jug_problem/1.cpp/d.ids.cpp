...
bool iterativeDeepeningSearch(State initialState, int maxDepth) {
    for (int depth = 0; depth <= maxDepth; ++depth) {
        set<pair<int, int>> visited;
        cout << "Searching with depth limit: " << depth << endl;
        if (depthLimitedSearch(initialState, 0, depth, visited)) {
            return true;
        }
    }
    return false;
...
int main() {
    ...
    int maxDepth = 20;  // You can adjust this limit based on the problem complexity

    if (!iterativeDeepeningSearch(initialState, maxDepth)) {
